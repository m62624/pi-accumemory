...
export class AboutDesk {
	...
	private currentSettings(): string {
		...
		const lines = [
			...
			`- project.markers: ${memory.project.markers.length === 0 ? "none, so only folders somebody asked for have a memory" : memory.project.markers.join(", ")}`,
			`- project.maxParents: ${memory.project.maxParents} (how far up the folder tree the search goes)`,
			"",
			"## Semantic search",
			...this.embedderLines(),
