 ...
 *
 * Pi lays widgets, selectors and status lines out with fixed-width math, so a line
 * longer than the terminal - or one carrying a tab, a stray ANSI escape, or a
 * control byte - desyncs the frame: narrow the window and the layout breaks. The
 * same defence pi-telegram-manager and pi-planner use: sanitize the text, then clip
 * each line to the width, with a floor so an absurdly narrow terminal still gets
 * something rather than a crash.
 *
 * It matters more here than it would in a widget of our own words, because a picker
 * row is built out of a FOLDER NAME - whatever the filesystem happens to hold, not
 * anything this extension chose.
 *
