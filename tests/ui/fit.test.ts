...
describe("fitLine", () => {
	it("leaves a line that fits untouched", () => {
		expect(fitLine("p_dd21d9 · myproj · 84 facts", 40)).toBe(
			"p_dd21d9 · myproj · 84 facts",
		);
	});

	it("clips an over-long line to the width, ellipsis included", () => {
		const fitted = fitLine("p_dd21d9 · myproj · /home/someone/code/myproj", 12);
	...
	it("never lays out below the floor, however narrow the terminal claims to be", () => {
		...
		expect([...fitLine("p_dd21d9 · myproj", 0)]).toHaveLength(8);
		expect([...fitLine("p_dd21d9 · myproj", -5)]).toHaveLength(8);
	});
