...
describe("detectProjectRoot", () => {
	it("finds the git root from a nested directory", () => {
		const found = detectProjectRoot("/home/m/app/src/deep", {
	...
	it("accepts a manifest as a marker too, for a project with no git", () => {
		expect(
			detectProjectRoot("/home/m/app/src", {
	...
	it("prefers the nearest marker when both an inner and an outer one exist", () => {
		// A package inside a monorepo is its own project: its conventions and
		// gotchas are not the root's.
		expect(
			detectProjectRoot("/home/m/mono/packages/api/src", {
				fs: fsWith(
					"/home/m/mono/.git",
	...
	it("returns undefined for a directory that is not a project", () => {
		...
		expect(
			detectProjectRoot("/home/m", { fs: fsWith(), flavour: posix }),
		).toBeUndefined();
		expect(
			detectProjectRoot("/tmp", { fs: fsWith(), flavour: posix }),
		).toBeUndefined();
	});

	it("stops at the filesystem root instead of looping", () => {
		expect(
			detectProjectRoot("/", { fs: fsWith(), flavour: posix }),
		).toBeUndefined();
	});

	it("walks up on windows too, and stops at the drive root", () => {
		const win = path.win32;
		expect(
			detectProjectRoot("C:\\Users\\m\\app\\src", {
				fs: fsWith("C:\\Users\\m\\app\\.git"),
				flavour: win,
			}),
		).toBe("C:\\Users\\m\\app");
		expect(
			detectProjectRoot("C:\\", { fs: fsWith(), flavour: win }),
		).toBeUndefined();
	});

	it("treats the marker directory itself as the root", () => {
		expect(
			detectProjectRoot("/home/m/app", {
				fs: fsWith("/home/m/app/.git"),
				flavour: posix,
			}),
		).toBe("/home/m/app");
	...
	it("lists git first among the markers", () => {
